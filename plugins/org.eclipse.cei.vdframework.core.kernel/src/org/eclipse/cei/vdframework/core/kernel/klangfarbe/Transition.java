...
public class Transition {
    ...
    public Transition(State start, State end, Event event, Guard guard, Action action) {
    ...
    boolean execute(Event event, Metadata data, Parameter parameter) {
        ...
        for (int i = 0; i < activate.size(); i++) {
            /*
             * check if we activate an concurrent state imlicit and if so make
             * sure adding the correct region to the list of regions to ignore
             * on activation. It is activated by this transition.
             */
    ...
    boolean allowed(Metadata data, Parameter parameter) {
         ...
         * this state. So get the last state in the activate list. end() is
         * behind the last element so we have to select the prior element by
         * using *--.
         */
        State target = activate.lastElement();
