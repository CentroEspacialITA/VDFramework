...
public class PseudoState extends State {
    ...
    boolean lookup(Metadata data, Parameter parameter) {
        ...
        if (type == pseudostate_join) {
            for (int i = 0; i < incoming.size(); i++) {
                ...
                if (d == null || d != null && !d.active || t.hasGuard() && !t.guard.check(data, parameter)) {
                    return false;
                }
            }
        }
    ...
    /**
     * Activates the state and executes the next nessecary steps depending on
     * the type. Overwrites the method from class State.
     */
    @Override
     ...
     * ignored because on activation the substates are in the context the
     * current active state and not the history pseudostate anymore. for other
     * states simply call the base method.
     */
    @Override
    ...
    private static void calculate(Vector<State> history, State s, Metadata data, int type) {
        ...
        if (s instanceof HierarchicalState) {
            StateRuntimedata runtimedata = data.getData(s);
        ...
        /*
         * if (s instanceof ConcurrentState) { Vector regions =
         * ((ConcurrentState) s).regions; for (int i = 0; i < regions.size();
         * i++) { calculate(history, (State) regions.get(i), data, type); } }
         */
    }
}
