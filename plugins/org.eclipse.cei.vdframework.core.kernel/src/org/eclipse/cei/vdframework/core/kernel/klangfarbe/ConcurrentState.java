...
public class ConcurrentState extends Context {
    ...
    boolean activate(Metadata data, Parameter parameter) {
        if (super.activate(data, parameter)) {
            ...
            for (int i = 0; i < regions.size(); i++) {
                // check if the region is activated implicit via a incoming
                // transition
    ...
    boolean dispatch(Metadata data, Event event, Parameter parameter) {
        ...
        /*
         * Dispatch the event in all regions as long as this state is active. If
         * we don not check this, an implicit exist would be ignored by this
         * code.
         */
